...
.. note::
  ...
  the `regenerate-spir-spirv target`_.
...
* ``nospir`` - Skip all SPIR generation steps for this kernel. This will only
  have an effect if you are running the `regenerate-spir-spirv target`_.
...
* ``nospirv`` - Skip all SPIR-V generation steps for this kernel. This will
  only have an effect if you are running the `regenerate-spir-spirv target`_.
...
`regenerate-spir-spirv target`_.
...
.. note::
  ...
  `regenerate-spir-spirv target`_) for CMake dependency tracking reasons. The
  ``install`` target contains logic to prevent stub kernels from being
  installed.
...
The `regenerate-spir-spirv target`_ is used to rebuild IR kernels. It is also
possible to build individual IR kernels `manually <Manually generate SPIR or
SPIR-V_>`_, but since the process is error-prone, committing manually generated
IR kernels to the repository is discouraged.

``regenerate-spir-spirv`` Target
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

There is a custom target ``regenerate-spir-spirv`` which can be used for
generating the appropriate SPIR BC and SPIR-V ASM files from all the kernels we
have in UnitCL. To use this target you will require Khronos' modified version
of Clang, the ``llvm-spirv`` tool and ``opencl_spir.h``.
...
With these set, calling the ``regenerate-spir-spirv`` target will build SPIR
and SPIR-V for all the kernels.

.. note::

  ``regenerate-spir-spirv`` compiles IR kernels into the source directory,
  **not** the build directory. However, temporary files are placed into the
  build directory (and later deleted).

``regenerate-spir-spirv`` will regenerate all binaries for all kernels, *even if
...
The following diagram shows how the ``regenerate-spir-spirv`` target works:
...
The ``regenerate-spir-spirv`` target is most useful for automating generation
of all the kernels. However, sometimes you might prefer to generate these
yourself by hand.
...
.. warning::
  ...
  though they have the same name. (``regenerate-spir-spirv`` places these
  temporary bitcode files in the build directory to avoid name clashes.)
