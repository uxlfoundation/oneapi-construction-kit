...
``OpenCL`` respectively in the form ``lib<name>.so`` or ``<name>.dll``,
depending on if the platform is Linux or Windows. The ICD loaders gets linked to
application code so that at runtime the ICD loader will select the
implementation of the standard to run and load the chosen shared library,
allowing multiple platforms to coexist on the same system.
