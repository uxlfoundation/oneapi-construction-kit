...
class CommandBufferUpdateNDKernel : public MutableDispatchTest {
 protected:
  ...
  virtual void SetUp() override {
    ...
    ASSERT_SUCCESS(clSetKernelArg(kernel, 0, sizeof(cl_mem), &src_buffer));
    ASSERT_SUCCESS(clSetKernelArg(kernel, 1, sizeof(cl_mem), &dst_buffer));
  }
...
TEST_F(CommandBufferUpdateNDKernel, InvalidNext) {
  ...
  cl_mutable_base_config_khr dispatch_config{
...
TEST_F(CommandBufferUpdateNDKernel, NopUpdate) {
  ...
  cl_mutable_base_config_khr nop_update{
...
TEST_F(CommandBufferSimultaneousUpdate, ConsecutiveUpdate) {
  ...
  cl_mutable_dispatch_arg_khr input_arg = {0, sizeof(cl_mem),
                                           &updated_src_buffer};
  cl_mutable_dispatch_config_khr dispatch_config{
      CL_STRUCTURE_TYPE_MUTABLE_DISPATCH_CONFIG_KHR,
      nullptr,
  ...
  cl_mutable_dispatch_arg_khr output_args = {1, sizeof(cl_mem),
                                             &updated_dst_buffer};
...
TEST_F(CommandBufferSimultaneousUpdate, MultipleCommands) {
  ...
  ASSERT_SUCCESS(clSetKernelArg(kernel, 0, sizeof(cl_mem), &dst_buffer));
  ASSERT_SUCCESS(
      clSetKernelArg(kernel, 1, sizeof(cl_mem), &updated_src_buffer));
  ...
  cl_mutable_dispatch_arg_khr arg1 = {1, sizeof(cl_mem), &updated_dst_buffer};
  cl_mutable_dispatch_arg_khr arg2 = {0, sizeof(cl_mem), &updated_dst_buffer};
