...
class clCreateProgramWithILTest : public ucl::ContextTest {
 public:
  void SetUp() override {
  ...
  void run_default_test() {
    ...
    ASSERT_SUCCESS(
        clGetProgramInfo(program, CL_PROGRAM_IL, size, il.data(), nullptr));
...
TEST_F(clCreateProgramWithILTest, Default) { run_default_test(); }

TEST_F(clCreateProgramWithILTest, clGetProgramInfo) {
  size_t size;
  ASSERT_SUCCESS(
      clGetDeviceInfo(device, CL_DEVICE_IL_VERSION, 0, nullptr, &size));
  std::string il_version(size, '\0');
...
TEST_F(clCreateProgramWithILTest, InvalidContext) {
  cl_int error;
  ASSERT_EQ(nullptr,
            clCreateProgramWithIL(nullptr, spirv.data(),
                                  spirv.size() * sizeof(spirv[0]), &error));
  ASSERT_EQ_ERRCODE(CL_INVALID_CONTEXT, error);
}

TEST_F(clCreateProgramWithILTest, InvalidValue) {
  cl_int error;
  ASSERT_EQ(nullptr,
            clCreateProgramWithIL(context, nullptr,
                                  spirv.size() * sizeof(spirv[0]), &error));
  ASSERT_EQ_ERRCODE(CL_INVALID_VALUE, error);
  ASSERT_EQ(nullptr, clCreateProgramWithIL(context, spirv.data(), 0, &error));
  ASSERT_EQ_ERRCODE(CL_INVALID_VALUE, error);
  std::array<uint32_t, 32> invalid{{}};
  ASSERT_EQ(nullptr,
            clCreateProgramWithIL(context, invalid.data(),
                                  invalid.size() * sizeof(spirv[0]), &error));
  ASSERT_EQ_ERRCODE(CL_INVALID_VALUE, error);
}

// The same as Default but with threads.
// See CA-2811.
TEST_F(clCreateProgramWithILTest, ConcurrentNDRangeKernel) {
  // 10 threads gives a failure in ~1/10 runs (before the fix was merged).
  UCL::vector<std::thread> workers(10);

  // Spawn those threads.
  for (size_t i = 0; i < workers.size(); i++) {
    workers[i] = std::thread([this] { this->run_default_test(); });
  }
