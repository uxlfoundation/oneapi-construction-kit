...
kernel void libm_native_double_input(__global double* in, __global uint* out) {
  uint index = 0;
  ...
  out[index] = native_divide(in[index], 1000.0) >= BIG_VALF ? 2 : 1; index++;
  out[index] = native_powr(in[index], 1.0) >= BIG_VALF ? 2 : 1; index++;
