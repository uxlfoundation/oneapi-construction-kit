...
// CL_STD: 3.0
__kernel void compare_exchange_strong_local_private_single_long(
    volatile __global long *inout_buffer, __global long *expected_buffer,
    __global long *desired_buffer, int __global *bool_output_buffer,
    __local volatile atomic_long *local_atomic) {
  int gid = get_global_id(0);
  int lid = get_local_id(0);
  int wgid = get_group_id(0);

  long expected_private = expected_buffer[gid];
