...
int main(int argc, char **argv) {
  ...
  parser.help([]() {
    fprintf(stderr, "Usage: ./vector_add [--local-size L] [--global-size S]\n");
  });
  ...
  clik_program *program = clik_create_program(device, vector_add_kernel_binary,
                                              vector_add_kernel_binary_size);
  if (!program) {
  ...
  if (!clik_write_buffer(device, src1_buffer, 0, &src1_data[0], buffer_size)) {
    fprintf(stderr, "Could not write to the src1 buffer.\n");
    return 4;
  }
  if (!clik_write_buffer(device, src2_buffer, 0, &src2_data[0], buffer_size)) {
    fprintf(stderr, "Could not write to the src2 buffer.\n");
    return 4;
  }
  ...
  printf("Running vector_add example (Global size: %zu, local size: %zu)\n",
  ...
  if (!clik_run_kernel(program, "kernel_main", &ndrange, &args[0], num_args)) {
