...
int main(int argc, char **argv) {
  ...
  parser.help([]() {
    fprintf(stderr, "Usage: ./matrix_multiply_tiled [--matrix-size M]\n");
  });
  ...
  clik_program *program =
      clik_create_program(device, matrix_multiply_tiled_kernel_binary,
                          matrix_multiply_tiled_kernel_binary_size);
  if (!program) {
  ...
  clik_init_ndrange_2d(&ndrange, M, M, TS, TS);

  printf(
      "Running matrix_multiply_tiled example (Global size: %zux%zu, "
      "local size: %zux%zu)\n",
