...
// 'device_vector_add_wfv.c' and turning it into a C array using the Bin2H tool.
...
int main(int argc, char **argv) {
  ...
  parser.help([]() {
    fprintf(stderr,
            "Usage: ./vector_add_wfv [--local-size L] [--global-size S]\n");
  });
  ...
  clik_program *program = clik_create_program(
      device, vector_add_wfv_kernel_binary, vector_add_wfv_kernel_binary_size);
  if (!program) {
  ...
  printf("Running vector_add_wfv example (Global size: %zu, local size: %zu)\n",
