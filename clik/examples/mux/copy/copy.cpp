...
// 'device_copy.c' and turning it into a C array using the Bin2H tool.
#include "kernel_binary.h"

// This header contains the kernel bitcode resulting from turning
// 'device_copy32.bc' or 'device_copy64.bc' into a C array using the Bin2H tool.
// These bitcode files will be compiled by Mux down to device assembly which
// performs the same operations as the 'copy' kernel in 'device_copy.c'.
#include "kernel_bitcode.h"

int main(int argc, char **argv) {
  // Process command line options.
  size_t local_size = 16;
  size_t global_size = 1024;
  bool online_compilation = false;
  option_parser_t parser;
  parser.help([]() {
    fprintf(stderr,
            "Usage: ./copy_mux [--local-size L] [--global-size S] "
            "[--online-compilation]\n");
  });
  ...
  parser.option(0, "online-compilation", 0,
                [&](const char *s) { online_compilation = true; });
  parser.parse(argv);
  if (local_size < 1) {
  ...
  if (online_compilation) {
    coreCreateExecutable(device, finalizer, core_source_type_llvm_100, {},
                         copy_kernel_bitcode, copy_kernel_bitcode_size,
                         allocator, &executable);
  } else {
    coreCreateExecutable(device, finalizer, core_source_type_binary, {},
                         copy_mux_kernel_binary, copy_mux_kernel_binary_size,
                         allocator, &executable);
  }
  if (!executable) {
  ...
  descriptors[0].buffer_descriptor.buffer = dst_buffer;
  descriptors[0].buffer_descriptor.offset = 0;
  descriptors[1].type = core_descriptor_info_type_buffer;
  descriptors[1].buffer_descriptor.buffer = src_buffer;
  ...
  printf("Running copy_mux example (Global size: %zu, local size: %zu)\n",
         global_size, local_size);
