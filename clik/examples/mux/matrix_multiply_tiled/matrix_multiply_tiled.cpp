...
// 'device_matrix_multiply_tiled.c' and turning it into a C array using the
// Bin2H tool.
...
int main(int argc, char **argv) {
  ...
  parser.help([]() {
    fprintf(stderr,
            "Usage: ./matrix_multiply_tiled_tiled_mux [--matrix-size M]\n");
  });
  parser.option('M', "matrix-size", 1,
                [&](const char *s) { matrix_size = strtoull(s, 0, 0); });
  parser.parse(argv);
  if ((matrix_size < 1) || (matrix_size % TS) != 0) {
    fprintf(stderr,
            "error: matrix size must be positive and a multiple of "
            "%d\n",
            TS);
    return 7;
  }
  ...
  coreCreateExecutable(device, finalizer, core_source_type_binary, {},
                       matrix_multiply_tiled_mux_kernel_binary,
                       matrix_multiply_tiled_mux_kernel_binary_size, allocator,
                       &executable);
  if (!executable) {
  ...
  coreCreateScheduledKernel(device, finalizer, kernel, TS, TS, 1, allocator,
                            &scheduled_kernel);
  ...
  printf(
      "Running matrix_multiply_tiled_mux example (Global size: %zux%zu, "
      "local size: %dx%d)\n",
      global_size[0], global_size[1], TS, TS);
