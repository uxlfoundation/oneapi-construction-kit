...
int main(int argc, char **argv) {
  ...
  parser.help([]() {
    fprintf(stderr,
            "Usage: ./vector_add_mux [--local-size L] [--global-size S]\n");
  });
  ...
  coreCreateExecutable(device, finalizer, core_source_type_binary, {},
                       vector_add_mux_kernel_binary,
                       vector_add_mux_kernel_binary_size, allocator,
                       &executable);
  if (!executable) {
  ...
  core_buffer_t src1_buffer = nullptr;
  core_buffer_t src2_buffer = nullptr;
  core_buffer_t dst_buffer = nullptr;
  core_memory_t memory = nullptr;
  coreCreateBuffer(device, buffer_size, allocator, &src1_buffer);
  coreCreateBuffer(device, buffer_size, allocator, &src2_buffer);
  ...
  coreBindBufferMemory(device, memory, dst_buffer, buffer_size * 0);
  coreBindBufferMemory(device, memory, src1_buffer, buffer_size * 1);
  coreBindBufferMemory(device, memory, src2_buffer, buffer_size * 2);
  ...
  corePushWriteBuffer(command_write, src1_buffer, 0, &src1_data[0],
                      buffer_size);
  corePushWriteBuffer(command_write, src2_buffer, 0, &src2_data[0],
                      buffer_size);
  ...
  printf("Running vector_add_mux example (Global size: %zu, local size: %zu)\n",
         global_size, local_size);
  ...
  coreDestroyBuffer(device, src1_buffer, allocator);
  coreDestroyBuffer(device, src2_buffer, allocator);
