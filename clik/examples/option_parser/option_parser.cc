...
void option_parser_t::option(char c, const char *s, int arg,
...
const char *const *option_parser_t::parse(const char *const *argv0) {
  ...
  for (const char *opt; (opt = *argv) != NULL && opt[0] == '-'; argv++) {
    ...
    for (auto it = opts.begin(); !found && it != opts.end(); it++) {
      ...
      bool str_match =
          opt[1] == '-' && slen && strncmp(opt + 2, it->str, slen) == 0;
      if (chr_match ||
          (str_match && (opt[2 + slen] == '=' || opt[2 + slen] == '\0'))) {
        const char *optarg =
            chr_match
                ? (opt[2] ? &opt[2] : NULL)
                : opt[2 + slen] ? &opt[3 + slen] : it->arg ? *(++argv) : NULL;
        if (optarg && !it->arg)
    ...
    if (!found) error("unrecognized option", *argv0, opt);
  }
  return argv;
}

void option_parser_t::error(const char *msg, const char *argv0,
                            const char *arg) {
