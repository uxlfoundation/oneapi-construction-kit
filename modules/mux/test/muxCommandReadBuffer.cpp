...
INSTANTIATE_DEVICE_TEST_SUITE_P(muxCommandReadBufferTest);

TEST_P(muxCommandReadBufferTest, Default) {
  char data[MEMORY_SIZE];

  ASSERT_SUCCESS(muxCommandReadBuffer(command_buffer, buffer, 0, data,
                                      MEMORY_SIZE, 0, nullptr, nullptr));
}

TEST_P(muxCommandReadBufferTest, WithOffset) {
  char data[MEMORY_SIZE];

  ASSERT_SUCCESS(muxCommandReadBuffer(command_buffer, buffer, 1, data, 1, 0,
                                      nullptr, nullptr));
}

TEST_P(muxCommandReadBufferTest, InvalidOffset) {
  char data[MEMORY_SIZE];

  ASSERT_ERROR_EQ(mux_error_invalid_value,
                  muxCommandReadBuffer(command_buffer, buffer, MEMORY_SIZE,
                                       data, 1, 0, nullptr, nullptr));
}

TEST_P(muxCommandReadBufferTest, InvalidOffsetPlusSize) {
  char data[MEMORY_SIZE];

  ASSERT_ERROR_EQ(mux_error_invalid_value,
                  muxCommandReadBuffer(command_buffer, buffer, 1, data,
                                       MEMORY_SIZE, 0, nullptr, nullptr));
}

TEST_P(muxCommandReadBufferTest, InvalidHostPointer) {
  ASSERT_ERROR_EQ(mux_error_invalid_value,
                  muxCommandReadBuffer(command_buffer, buffer, 0, 0, 1, 0,
                                       nullptr, nullptr));
}

TEST_P(muxCommandReadBufferTest, ZeroSize) {
  char data[MEMORY_SIZE];

  ASSERT_ERROR_EQ(mux_error_invalid_value,
                  muxCommandReadBuffer(command_buffer, buffer, 0, data, 0, 0,
                                       nullptr, nullptr));
}

TEST_P(muxCommandReadBufferTest, InvalidSize) {
  char data[MEMORY_SIZE * 2];

  ASSERT_ERROR_EQ(mux_error_invalid_value,
                  muxCommandReadBuffer(command_buffer, buffer, 0, data,
                                       MEMORY_SIZE * 2, 0, nullptr, nullptr));
}

TEST_P(muxCommandReadBufferTest, Sync) {
  char data[MEMORY_SIZE];
