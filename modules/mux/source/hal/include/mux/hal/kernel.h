...
template <class VariantData> struct kernel : mux_kernel_s {
  ...
  kernel(mux::hal::device *device, cargo::string_view name,
      ...
      : name(cargo::as<std::string>(name)), object_code(object_code),
        allocator(allocator), variant_data(std::move(variant_data)) {
    this->device = device;
  }

  template <class Kernel>
  static cargo::expected<Kernel *, mux_result_t>
  create(mux::hal::device *device, mux::hal::executable *executable,
