...
; CHECK: entry:
; CHECK:   %[[LD:.*]] = load float
; CHECK:   %[[INS0:.*]] = insertelement <4 x float> {{undef|poison}}, float %[[LD]], i32 0
; CHECK-NOT: %{{.*}} = insertelement <4 x float> %{{.*}}, float %[[LD]], i32 1
; CHECK-NOT: %{{.*}} = insertelement <4 x float> %{{.*}}, float %[[LD]], i32 2
; CHECK-NOT: %{{.*}} = insertelement <4 x float> %{{.*}}, float %[[LD]], i32 3
