...
template <typename T> T cospi(const T x) {
  const T xAbs = __abacus_fabs(x);

  // Find the fractional component of x and subtract 0.5 to turn cos into sin:
  // cos(pi * x) = sin((pi * x) - (pi * 0.5))
  const T xfract = (xAbs - __abacus_floor(xAbs)) - (T)0.5;

  // Calculate polynomial of 'sin(pi * sqrt(x)) / sqrt(x)' over input range
  // [ 1e-50, (0.5)^2 ]. See sinpi.sollya or sinpidouble maple worksheet, since
  // the polynomial is the same as sinpi.
  const T poly = helper<T>::_(xfract * xfract);
  const T result = xfract * poly;
...
abacus_half2 ABACUS_API __abacus_cospi(abacus_half2 x) { return cospi<>(x); }
abacus_half3 ABACUS_API __abacus_cospi(abacus_half3 x) { return cospi<>(x); }
abacus_half4 ABACUS_API __abacus_cospi(abacus_half4 x) { return cospi<>(x); }
abacus_half8 ABACUS_API __abacus_cospi(abacus_half8 x) { return cospi<>(x); }
abacus_half16 ABACUS_API __abacus_cospi(abacus_half16 x) { return cospi<>(x); }
#endif // __CA_BUILTINS_HALF_SUPPORT

abacus_float ABACUS_API __abacus_cospi(abacus_float x) { return cospi<>(x); }
abacus_float2 ABACUS_API __abacus_cospi(abacus_float2 x) { return cospi<>(x); }
abacus_float3 ABACUS_API __abacus_cospi(abacus_float3 x) { return cospi<>(x); }
abacus_float4 ABACUS_API __abacus_cospi(abacus_float4 x) { return cospi<>(x); }
abacus_float8 ABACUS_API __abacus_cospi(abacus_float8 x) { return cospi<>(x); }
abacus_float16 ABACUS_API __abacus_cospi(abacus_float16 x) {
  return cospi<>(x);
}

#ifdef __CA_BUILTINS_DOUBLE_SUPPORT
abacus_double ABACUS_API __abacus_cospi(abacus_double x) { return cospi<>(x); }
abacus_double2 ABACUS_API __abacus_cospi(abacus_double2 x) {
  return cospi<>(x);
}
abacus_double3 ABACUS_API __abacus_cospi(abacus_double3 x) {
  return cospi<>(x);
}
abacus_double4 ABACUS_API __abacus_cospi(abacus_double4 x) {
  return cospi<>(x);
}
abacus_double8 ABACUS_API __abacus_cospi(abacus_double8 x) {
  return cospi<>(x);
}
