...
; RUN: muxc --device "%riscv_device" --passes optimal-builtin-replace,verify -S %s \
; RUN:   | FileCheck %s --check-prefix CHECK-NO-ANALYSIS
; RUN: muxc --device "%riscv_device" --passes "require<builtin-info>,optimal-builtin-replace,verify" -S %s \
