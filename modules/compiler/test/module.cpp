...
/// @brief Test fixture for testing behaviour of the
/// compiler::Module::setSnapshotCallback API.
struct SetSnapshotCallbackTest : OpenCLCModuleTest {
  // This won't be called as program is never built/enqueued
  static void unitcompiler_snapshot_callback(size_t snapshot_size,
                                             const char *snapshot_data,
                                             void * /*callback_data*/,
                                             void * /*user_data*/) {
    EXPECT_TRUE(nullptr != snapshot_data);
    EXPECT_TRUE(0 != snapshot_size);
  }
};

TEST_P(SetSnapshotCallbackTest, SetSnapshotValidStage) {
  uint32_t num_stages = 0;
  ASSERT_EQ(compiler::Result::SUCCESS,
            target->listSnapshotStages(0, nullptr, &num_stages));

  int callback_data = 0;
  auto callback = &SetSnapshotCallbackTest::unitcompiler_snapshot_callback;
  if (num_stages > 1) {
    const char *stages[1] = {nullptr};
    ASSERT_EQ(compiler::Result::SUCCESS,
              target->listSnapshotStages(1, stages, nullptr));
    ASSERT_NE(nullptr, stages[0]);

    EXPECT_EQ(compiler::Result::SUCCESS,
              module->setSnapshotCallback(stages[0], callback, nullptr,
                                          compiler::SnapshotFormat::DEFAULT));
    EXPECT_EQ(compiler::Result::SUCCESS,
              module->setSnapshotCallback(stages[0], callback, nullptr,
                                          compiler::SnapshotFormat::TEXT));
    EXPECT_EQ(compiler::Result::SUCCESS,
              module->setSnapshotCallback(stages[0], callback, nullptr,
                                          compiler::SnapshotFormat::BINARY));

    EXPECT_EQ(
        compiler::Result::SUCCESS,
        module->setSnapshotCallback(stages[0], callback, (void *)&callback_data,
                                    compiler::SnapshotFormat::DEFAULT));
    EXPECT_EQ(
        compiler::Result::SUCCESS,
        module->setSnapshotCallback(stages[0], callback, (void *)&callback_data,
...
TEST_P(SetSnapshotCallbackTest, InvalidSetSnapshotArgs) {
  uint32_t num_stages = 0;
  ASSERT_EQ(compiler::Result::SUCCESS,
            target->listSnapshotStages(0, nullptr, &num_stages));
  ...
  if (num_stages > 1) {
    ...
    EXPECT_EQ(compiler::Result::INVALID_VALUE,
              module->setSnapshotCallback(stages[0], nullptr, nullptr,
                                          compiler::SnapshotFormat::DEFAULT));
    EXPECT_EQ(compiler::Result::INVALID_VALUE,
              module->setSnapshotCallback(stages[0], nullptr, nullptr,
                                          compiler::SnapshotFormat::TEXT));
  ...
  auto callback = &SetSnapshotCallbackTest::unitcompiler_snapshot_callback;
  EXPECT_EQ(compiler::Result::INVALID_VALUE,
            module->setSnapshotCallback(nullptr, callback, nullptr,
                                        compiler::SnapshotFormat::DEFAULT));
  EXPECT_EQ(compiler::Result::INVALID_VALUE,
            module->setSnapshotCallback(nullptr, callback, nullptr,
                                        compiler::SnapshotFormat::TEXT));
...
INSTANTIATE_COMPILER_TARGET_TEST_SUITE_P(SetSnapshotCallbackTest);
